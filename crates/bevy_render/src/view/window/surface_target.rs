...
pub enum SurfaceTargetError {
    ...
    NoSurfaceTarget,
    #[error("This surface can only be accessed from the main thread")]
    InvalidThread,
}
...
pub enum SurfaceCreationError {
    #[error("SurfaceTargetSource did not return a surface target")]
    NoSurfaceTarget,
    #[error("This surface can only be created from the main thread")]
    InvalidThread,
...
impl RenderSurface {
    ...
    /// ## Safety
    ///
    /// The caller must ensure the surface is only used when the window / view is still alive and valid.
...
impl<T: HasSurfaceTarget + 'static> HasSurfaceTarget for NonSendHasSurfaceTarget<T> {
    fn surface_target(&self) -> Option<SurfaceTargetWrapper> {
...
impl SurfaceTargetSource {
    ...
    pub fn surface_target(
        ...
        self.source
            .surface_target()
            .ok_or(SurfaceTargetError::NoSurfaceTarget)
    }
...
/*
/// Stores the resolved surface target applied to [`bevy_window::Window`] entities.
///
/// Formerly known as `RawHandleWrapper`.
#[derive(Component, Clone)]
pub struct RenderSurfaceTarget {
    /// Holds a reference to the source window or view that provided the surface. Holding this ensures that the surface remains valid for the lifetime of the window.
    pub(crate) _source: Arc<dyn HasSurfaceTarget + Send + Sync + 'static>,
    /// Holds a reference to the surface target.
    pub(crate) surface_target: Arc<SurfaceTargetWrapperSendSync>,
}

impl RenderSurfaceTarget {
    /// This will return `None` if the surface target source does not return a surface target.
    pub(crate) fn new(surface_target_source: &SurfaceTargetSource) -> Option<Self> {
        // SAFETY: TODO
        let surface_target = (unsafe { surface_target_source.surface_target() })?;
        let static_surface_target = unsafe {
            std::mem::transmute::<SurfaceTargetWrapper<'_>, SurfaceTargetWrapper<'static>>(
                surface_target,
            )
        };
        Some(Self {
            _source: surface_target_source.source.clone(),
            surface_target: Arc::new(SurfaceTargetWrapperSendSync(static_surface_target)),
        })
    }
}

impl HasDisplayHandle for RenderSurfaceTarget {
    fn display_handle(&self) -> Result<DisplayHandle<'_>, wgpu::rwh::HandleError> {
        self.surface_target.0.display_handle()
    }
}

impl HasWindowHandle for RenderSurfaceTarget {
    fn window_handle(&self) -> Result<WindowHandle<'_>, wgpu::rwh::HandleError> {
        self.surface_target.0.window_handle()
    }
}

/// An internal type that allows [`SurfaceTargetWrapper`] to be sent across threads.
pub(crate) struct SurfaceTargetWrapperSendSync(SurfaceTargetWrapper<'static>);

unsafe impl Send for SurfaceTargetWrapperSendSync {}
unsafe impl Sync for SurfaceTargetWrapperSendSync {}
*/
